...
public function getAssessedCandidate()
{
    return DB::table('competency_interviews as ci')
        ->select([
            ...
            'ci.pregnancy_months',
            'ci.reference_check',
            'ci.reference_remarks',
            'ci.current_packages',
            'ci.agreed_salary',
            'ci.required_notes',
            'ci.education_knowledge_remark',
            'ci.relevant_experience_remark',
            'ci.major_achievement_remark',
            'ci.language_fluency_remark',
            'ci.surgery_operation_remark',
            'jt.name as job_title',
            'cc.name as cost_center',

            // Ratings
            DB::raw("CASE ci.education_knowledge
                        WHEN 0 THEN 'N/A (0)'
                        WHEN 1 THEN 'Below Average (1)'
                        WHEN 2 THEN 'Average (2)'
                        WHEN 3 THEN 'Good (3)'
                        WHEN 4 THEN 'V.Good (4)'
                        ELSE 'Outstanding (5)' END AS education_knowledge"),

            DB::raw("CASE ci.relevant_experience
                        WHEN 0 THEN 'N/A (0)'
                        WHEN 1 THEN 'Below Average (1)'
                        WHEN 2 THEN 'Average (2)'
                        WHEN 3 THEN 'Good (3)'
                        WHEN 4 THEN 'V.Good (4)'
                        ELSE 'Outstanding (5)' END AS relevant_experience"),

            DB::raw("CASE ci.major_achievement
                        WHEN 0 THEN 'N/A (0)'
                        WHEN 1 THEN 'Below Average (1)'
                        WHEN 2 THEN 'Average (2)'
                        WHEN 3 THEN 'Good (3)'
                        WHEN 4 THEN 'V.Good (4)'
                        ELSE 'Outstanding (5)' END AS major_achievement"),

            DB::raw("CASE ci.language_fluency_id
                        WHEN 0 THEN 'N/A (0)'
                        WHEN 1 THEN 'Below Average (1)'
                        WHEN 2 THEN 'Average (2)'
                        WHEN 3 THEN 'Good (3)'
                        WHEN 4 THEN 'V.Good (4)'
                        ELSE 'Outstanding (5)' END AS language_fluency_id"),

            DB::raw("CASE ci.overall_rating
                        WHEN 0 THEN 'N/A'
                        WHEN 1 THEN 'Below Average'
                        WHEN 2 THEN 'Average'
                        WHEN 3 THEN 'Good'
                        WHEN 4 THEN 'V.Good'
                        ELSE 'Outstanding' END AS overall_rating"),

            // ✅ Boolean conversions (using string comparisons for production compatibility)
            DB::raw("CASE WHEN ci.current_employed_entity::text = '1' THEN 'Private Sector' ELSE 'Public Sector' END AS current_employed_entity"),
            DB::raw("CASE WHEN ci.military_service::text = '1' THEN 'Yes' ELSE 'No' END AS military_service"),
            DB::raw("CASE WHEN ci.relative_inside::text = '1' THEN 'Yes' ELSE 'No' END AS relative_inside"),
            DB::raw("CASE WHEN ci.chronic_disease::text = '1' THEN 'Yes' ELSE 'No' END AS chronic_disease"),
            DB::raw("CASE WHEN ci.pregnant::text = '1' THEN 'Yes' ELSE 'No' END AS pregnant"),
            DB::raw("CASE WHEN ci.employed_before::text = '1' THEN 'Yes' ELSE 'No' END AS employed_before"),
            DB::raw("CASE WHEN ci.reference_check::text = '1' THEN 'Yes' ELSE 'No' END AS reference_check"),
            DB::raw("CASE WHEN ci.social_insuarance_status::text = '1' THEN 'Yes' ELSE 'No' END AS social_insuarance_status"),
            DB::raw("CASE WHEN ci.work_site::text = '1' THEN 'Yes' ELSE 'No' END AS work_site"),
            DB::raw("CASE WHEN ci.reallocation_place::text = '1' THEN 'Yes' ELSE 'No' END AS reallocation_place"),
            DB::raw("CASE WHEN ci.downloaded::text = '1' THEN 'Yes' ELSE 'No' END AS downloaded"),
            DB::raw("CASE WHEN ci.uploaded::text = '1' THEN 'Yes' ELSE 'No' END AS uploaded"),
            DB::raw("CASE ci.recruiter_recommendations
                        WHEN 1 THEN 'Accepted'
                        WHEN 2 THEN 'Not Accepted'
                        ELSE 'Waiting List' END AS recruiter_recommendations"),
            DB::raw("CASE WHEN ci.surgery_operation::text = '1' THEN 'Yes' ELSE 'No' END AS surgery_operation"),

            // Competency scores (example: you can repeat for all 18+ scores)
            DB::raw("CASE cots.interactive_communication
                        WHEN 0 THEN 'N/A (0)'
                        WHEN 1 THEN 'Below Average (1)'
                        WHEN 2 THEN 'Average (2)'
                        WHEN 3 THEN 'Good (3)'
                        WHEN 4 THEN 'V.Good (4)'
                        ELSE 'Outstanding (5)' END AS interactive_communication"),

            DB::raw("CASE cots.accountability
                        WHEN 0 THEN 'N/A (0)'
                        WHEN 1 THEN 'Below Average (1)'
                        WHEN 2 THEN 'Average (2)'
                        WHEN 3 THEN 'Good (3)'
                        WHEN 4 THEN 'V.Good (4)'
                        ELSE 'Outstanding (5)' END AS accountability"),
        ...
        ->whereIn('ci.recruiter_recommendations', [1, 2])
    ...
    public function showDownloadDetails()
    {
        return  DB::table('competency_interviews as ci')
            ->select([
                ...
                DB::raw("CASE WHEN ci.downloaded::text = '1' THEN 'Yes' ELSE 'No' END AS downloaded "),
                DB::raw("CASE WHEN ci.uploaded::text = '1' THEN 'Yes' ELSE 'No' END AS uploaded "),
                DB::raw("CASE WHEN ci.recruiter_recommendations::text = '1' THEN 'Accepted' WHEN ci.recruiter_recommendations::text = '2' THEN 'Not Accepted' ELSE 'Waiting List' END AS recruiter_recommendations "),
                DB::raw("CASE WHEN ci.surgery_operation::text = '1' THEN 'Yes' ELSE 'No' END AS surgery_operation "),
